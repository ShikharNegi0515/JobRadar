...
export class JobsService {
  ...
  async findAll(options: JobQueryOptions = {}) {
    ...
    const qb: SelectQueryBuilder<JobPost> = this.jobPostsRepository
  ...
  async findById(id: string) {
    const job = await this.jobPostsRepository
      .createQueryBuilder('job')
      .leftJoinAndSelect('job.skills', 'skill')
      // 24-hour rule enforced even on single fetch
      .where('job.id = :id', { id })
  ...
  async seedMockJobs(): Promise<void> {
    ...
    const mockJobs = [
      ...
      },
      {
        ...
        location: 'Pune',
        work_mode: 'ONSITE' as any,
        employment_type: 'INTERNSHIP' as any,
        experience_min: 0,
        experience_max: 0,
        salary_min: 15000,
        salary_max: 25000,
